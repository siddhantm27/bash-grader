...
def generate_report_card(name,roll_no,exams,student_marks,percentiles_list,grade,all):
...
with open("./main.csv", mode ='r') as csv_file:
    ...
    for row in marks:
        ...
        if marks.line_num==1:
            ...
            grade=decide_grade(student_marks,max,rubrics,grades)
            
            if all_report_cards:
                generate_report_card(name,roll_no,exams,student_marks,percentiles_list,grade,all=True)
            else:
                if user_choice==roll_no:
                    generate_report_card(name,roll_no,exams,student_marks,percentiles_list,grade,all=False)
