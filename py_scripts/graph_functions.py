...
def histogram(file):
    ...
    plt.axvline(
        ...
        color="green",
        linestyle="--",
        label=f"90th Percentile={round(perc_90,2)}",
    )
    plt.axvline(
        x=perc_75,
        color="orange",
        linestyle="--",
        label=f"75th Percentile={round(perc_75,2)}",
    )
    plt.axvline(
        x=mean_marks, color="red", linestyle="--", label=f"Mean={round(mean_marks,2)}"
    )
    plt.legend()
    plt.show()
...
def scatter_plot(file):
    ...
    plt.xticks(np.arange(1, len(marklist) + 26, 25))
    plt.xlabel("Rank")
    plt.ylabel("Marks")
    ...
    plt.axhline(
        y=mean_marks, color="red", linestyle="--", label=f"Mean={round(mean_marks,2)}"
    )
    plt.legend()
